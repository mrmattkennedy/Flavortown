...
class GoogleMapPlotter(object):
    ...
    def __init__(self, center_lat, center_lng, zoom, apikey=''):
        ...
        #self.coloricon = os.path.join(os.path.dirname(__file__), 'markers/%s.png') #old
    ...
    def geocode(self, location_string):
        ...
        key = 'AIzaSyC-riBWDqa4oOwr2xm8o295zqyzQ18_aFk'
        geocode = requests.get(
            'https://maps.googleapis.com/maps/api/geocode/json?key=%s&address="%s"' %(key, location_string))    
    ...
    def write_point(self, f, lat, lon, color, title):
        try:
            ...
            f.write("\t\tmarker.addListener('click', this.get_title);\n")
            f.write("\t\tgmarkers.push(marker);\n")
            f.write("\t\tbar.value += (1/total);\n")
            f.write('\n')
        except Exception:
            print(title)
